...
output:
  xaringan::moon_reader:
    ...
    nature:
      slideNumberFormat: "%current%/17" #/%total%"
...
- Tax evasion is **widespread** and **significant** .small[(Slemrod, 2019)]

  - Developing countries: Long-standing concern; Developed: growing (2008) 
  - US: Tax-gap 18.3% (2008-2010) .small[(IRS, 2016) [&#9654; Stats](#tax-stats)]
  
--
count: false
- **Measuring** tax evasion is **non-trivial**
  - Direct: unreliable .small[Surveys: Concealing behavior] </br> 
  &emsp; &emsp; &ensp; time and resource-consuming .small[Admin data; Experiments]
  - Indirect: **productivity** must be considered
...
--
count: false
- **RQ:** New estimation **strategy** to recover **tax evasion and productivity**
...
- Tax evasion is often Treated as a classical measurement error .small[(e.g., Blalock et al., 2004, p.204)]
  - correlated with the firm's characteristics; 
  - not mean zero
...
---
name: over-r
### Evasion by input overreporting

- More **relevant** than we thought .small[Ecuador: [&#9654; (Carrillo et al. 2022)](#carrillo)]

  - Quantitatively large .small[up to 14% of total purchases]
  - Not unique to small firms .small[vs. Self-employed, smallest firm]
  - Very large firms do not overreport inputs .small[e.g., Profit shifting (Bustos et al., 2022)]
...
--
count: false
- Largely **overlooked** by the literature .small[Revenue-underreporting focused]

???

- Up to 14% of a firm's annual purchases
- Larger firms actively overreport inputs
- Very large corporations can use more sophisticated avenues of tax avoidance that do not require evasion using fake receipts. Like profit shifting, transfers to lower-tax countries (Bustos et al., 2022)
...
- Tax evasion: .small[[&#9654; Lit conventions](#tax-conv) [&#9654; Lit Overview](#tax-stats) [&#9654; Corporate tax evasion](#firms-evasion)]
  ...
  - Cost overreporting .small[(Zumaya et al., 2021; Carrillo et al., 2022)]
--
count: false
- Production function and productivity estimation
  
  - Bias due to tax evasion .small[Productivity (Gandhi, Navarro, and Rivers, 2020 [GNR]; Ackerberg et al., 2015 [ACF]); Productivity gap (Syverson, 2011)]
  - Systematic overreporting of intermediate inputs .small[Measurement error on capital (Collard et al., 2020; Ursenbach, 2022)]
...
Intermediate inputs -> flexible input assumption on productivity
...
--
count: false
1. Contrast the **productivity distribution** with the mismeasured one .small[Bias magnitude, distribution dispersion, and ranking]
...
$Y_{it}=G(X_{it})e^{\omega_{it}+\varepsilon^Y_{it}}$

- We are also interested in the Markov process of productivity (AR1)

$\omega_{it}=\delta_0 + \delta_1 \omega_{it-1}+\eta_{it}$
...
- Assumptions: .small[[&#9654; Empirical evidence](#evidence)]
  ...
  - (A-2) $|\omega \rightarrow x^* \rightarrow \varepsilon^X | \varepsilon^Y$ Firms choose overreporting $\varepsilon^X_{it}$ </br>
    *after* productivity shock $\omega_{it}$ is realized and 
    true inputs $x^*_{it}$ are chosen but </br>
    *before* the output shock $\varepsilon^Y_{it}$
  .small[ [&#9654;Microfoundation](#model) [&#9654; Justification](#a-2)]
...
Let $G(X^*_{it})e^{\omega_{it}+\varepsilon^Y_{it}}=X^{*\beta}_{it}e^{\omega_{it}+\varepsilon^Y_{it}}$
...
---
### 2nd stage might fail

Can only evaluate GNR's 2nd stage for $S_{it}>s$
...
However, it will not work because of the selection on $S$, i.e., 
...
\begin{align}
  ...
  +\underbrace{\mathbb{Var}[\varepsilon^Y_{it}]}_{known} \;&\text{(A-2)}\\
\end{align}

.small[Note: (A-2) is not needed for productivity and true inputs]

---
### The power of deconvolution

- I can learn the **distribution of tax evasion** and how it has changed **over time**

- I can do the same with true inputs $X_{it}^*$ and productivity $\omega_{it}$

- Furthermore, I can get any mixed moment $\mathbb{E}[(\mathcal{Z}^c, \mathcal{Z}^d)^n|t]$
...
1. Firm-level data (output, inputs, prices) .small[[&#9654; Sum-stats](#sum-stats) [&#9654; Access](#data-slide)]

  - Mexico, Ecuador, Colombia, and Chile
  
2. Firm's tax records (transaction level)
  - Mexico and Ecuador .small[[&#9654; Contingency plan](#contingent)]
...
Applications
- The effect of the e-invoice on tax evasion through cost overreporting
...
- Measuring misallocations
  - Aggregate productivity decomposition à la Olley and Pakes (1986) and </br> the productivity bias due to tax evasion
  
Extensions  
- Non-linear production functions .small[Hu et al. (2022) framework [&#9654; preview](#hu-sch)]
- Misreporting of input prices
...
2. The effect of the e-invoice on input overreporting
...
- Future work:
  - Mismeasuring misallocations
  - Stochastic Bertrand Rationalizability and multiproduct firms
