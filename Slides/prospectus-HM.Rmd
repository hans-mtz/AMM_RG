...
- Tax evasion is a **widespread** phenomenon: 
  ...
  - Recent increased attention in developed countries (2008 crisis) .small[(Slemrod, 2019) [&#9654; Stats](#tax-stats)]
  
- Ignoring tax evasion leads to **biased** estimates of **productivity**
  - Likely to be correlated with the firm's characteristics and 
  - not mean zero

- I propose a new method to _**estimate productivity in the presence of tax evasion**_
  - Byproduct: firm-level tax evasion
...
- Structural approach leveraging from firm's optimal behavior (Western's legacy)
...
1. Derive a method to estimate **productivity** in the presence of tax evasion
   
2. Implement the method using data from developing countries
   
3. Estimate firm-level **tax evasion** considering their productivity
   
4. Study the **policy** implications
...
- Assume CD prod. fn. and panel data: $Y_{it}=X_{it}^\beta e^{\omega_{it}+\varepsilon^Y_{it}}$

- Reported inputs $X_{it}=X^*_{it}+\varepsilon^X_{it}$

- Productivity naively estimated in the presence of tax evasion $\tilde\omega_{it}$ and 
...
\begin{align*}
 \mathbb{E}[\tilde\omega_{it}|x]-\mathbb{E}[\omega_{it}|x]& = \beta \mathbb{E}\left[\ln\left(\frac{X^*_{it}}{X^*_{it}+\varepsilon^X_{it}}\right)|x\right] \\
...
1. Firm's tax records (transaction level)
   1. Mexico and Ecuador
2. Firm-level data (output, inputs, prices)
   1. Mexico, Ecuador, Colombia, and Chile
...
- Assumptions:
  - (A-1) Firms at the top of the size distribution do not overreport in inputs .small[Carrillo et al. (2022)]
...
Works because $\mathbb{E}[\varepsilon^Y_{it}|S_{it}>s]=\mathbb{E}[\varepsilon^Y_{it}]=0$

.small[[&#9654; ACF](#id-sources)]


---
### GNR 2nd stage might fail
...
\begin{align}
    ...
    \\
    \mathbb{E}[\mathcal Y_{it}|\mathcal Y_{it-1},S_{it}>s] &= \mathbb{E}[\omega_{it}|\omega_{it-1},S_{it}>s] \;&\text{(A-1)}
\end{align}

It will not work if selection on $S$, i.e., $\mathbb{E}[\omega_{it}|S_{it}>s]\not=\mathbb{E}[\omega_{it}]$
...
I can get all the moments for the tax evasion distribution, e.g., 
...
.small[[&#9654; Using instruments](#gnr-iv)]
.small[[&#9654; Using second moments](#sec-mom)]
...
- Allow for Translog production function
