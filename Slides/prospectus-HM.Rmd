...
- Tax evasion is **widespread** and **significant** .small[(Slemrod, 2019)]
  ...
  - Growing concern for developed countries (2008) 
  - US: Tax-gap 18.3% (2008-2010) .small[(IRS, 2016) [&#9654; Stats](#tax-stats)]
--
count: false
- Tax evasion is **hard to measure**
  - Concealing behavior from firms/individuals (Surveys)
...
- More **relevant** than we thought .small[Ecuador: [&#9654; (Carrillo et al. 2022)](#carrillo)]
  ...
  - Not unique to small firms .small[Self-employed, smallest firm]
  - Very large firms do not evade by overreporting .small[Profit shifting (Bustos et al., 2022)]
...
- Production function estimation
  ...
  - Bias due to tax evasion .small[Productivity (Gandhi, Navarro, and Rivers, 2020 [GNR]; Ackerberg et al., 2015 [ACF])]
  - Wider dispersion (is ranking preserved?)  .small[Productivity gap (Syverson, 2011)]
  - Systematic overreporting of intermediate inputs .small[Capital measurement error (Collard et al., 2020; Ursenbach's JMP)]
...
1. Study the **policy** implications
  - **Effect of e-invoice** on cost overrerporting .small[(OECD,2017; Castro et al.,2016)] 
  - **Reallocation** of resources from low- to high-productivity firms .small[(Eslava et al., 2004; Levy, 2018)]
...
3. Contrast the **productivity distribution** with the mismeasured one .small[Magnitude of the bias, dispersion of the distribution, and ranking] &#128679;
...
- Reported inputs are true inputs $X^*$ inflated by $e^{\varepsilon^X_{it}}$ to evade taxes
...
- Assumptions: .small[[&#9654; Empirical evidence](#evidence)]
  ...
  - (A-2) Overreporting is chosen after productivity shock is realized and true inputs are chosen but before the current-period exogenous shock 
  .small[ [&#9654; microfoundation](#model)]
...
because $\mathbb{Var}[\varepsilon^Y_{it}|S_{it}>s]=\mathbb{Var}[\varepsilon^Y_{it}|t]=\mathbb{Var}[\varepsilon^Y_{it}]$ or any other moment

\begin{align}
  \mathbb{E}[\mathcal E_{it}|t]&=\mathbb{E}[\varepsilon^X_{it}|t]-\mathbb{E}[\varepsilon^Y_{it}]\\
  &=\mathbb{E}[\varepsilon^X_{it}|t] \\
  \\
  \mathbb{Var}[\mathcal E_{it}|t]&=\mathbb{Var}[\varepsilon^X_{it}-\varepsilon^Y_{it}|t]\\
  &=\mathbb{Var}[\varepsilon^X_{it}|t]+\mathbb{Var}[\varepsilon^Y_{it}] \;&\text{(A-2)}\\
\end{align}
...
\begin{align}
  \mathbb{E}[\mathcal{X}_{it}|t]&=\mathbb{E}[\ln X^*_{it}|t]+\mathbb{E}[\varepsilon^Y_{it}]\\
  &=\mathbb{E}[\ln X^*_{it}|t] \\
  \\
  \mathbb{Var}[\mathcal{X}_{it}|t]&=\mathbb{Var}[\ln X^*_{it}|t]+Var[\varepsilon^Y_{it}]\\
\end{align}
...
- First, productivity shocks $\omega_{it}$ are realized. Firms observe input $\rho_t$ and output $P$ prices

- Firms choose optimal inputs given productivity, prices, and the production function

\begin{equation*}
  \max_{x_{it}} P_t\mathbb{E}[ x_{it}^\beta e^{\omega_{it}+\varepsilon_{it}}]-\rho_t x_{it}
\end{equation*}
...
- Then, firms choose input overreporting $\varepsilon^X$ given their true expected profits $P\mathbb{E}[y]-\rho x^*$ and tax rate $\tau$. 
  
- Firms get caught with probability $p(\varepsilon^X)$ and are punished with cost $\kappa$

\begin{align*}
  \max_{\varepsilon^X \in [0,\infty)} [1-p(\varepsilon^X)]&\left[(P\mathbb{E}[y]-\rho x^*) -\tau[P\mathbb{E}[y]-\rho(x^*+\varepsilon^X)]\right]\\
  +p(\varepsilon^X)&\left[(1-\tau)(P\mathbb{E}[y]-\rho x^*)-\kappa\right]
\end{align*}
...
.footer[[&#9664; Back to Bias](#bias) [&#9664; Back to Id-Strat](#id-strat)]
