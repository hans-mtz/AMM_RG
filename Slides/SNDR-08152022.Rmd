...
2. Firms' characteristics correlated with tax evasion: 
  1. Correlated with productivity: Revenue, market shares .small[(Carrillo et al., 2022)], age .small[(Zumaya et at., 2021)], relationship between costs and assets .small[(Gonzales and Velasques, 2013)]
  2. Uncorrelated with productivity: Owner's income .small[(Carrillo et al., 2022)], previous audits .small[(Gonzalez and Velasquez, 2013)]
...
- Assumptions: 
  ...
  2. Firms selling only to other firms (firm-to-firm) do not underreport revenue
  3. Firms overreport only input quantities, not prices (will be relaxed later)
  4. (Intermediate) input is flexible
  5. Demand of the true input is strictly monotone in productivity
  6. Evasion decision after productivity shock is realized and true inputs chosen
...
\begin{align*}
  \ln Y_{it}&=\beta\ln(X^*_{it}+\varepsilon^X_{it}) + \omega_{it} + \varepsilon^Y_{it} \\
  \ln Y_{it}&=\beta\ln(X^*_{it}+\varepsilon^X_{it}) + X^{-1}(X^{*-1}_{it}(\omega_{it})+\varepsilon^X_{it}) + \varepsilon^Y_{it} \\
  \ln Y_{it}&=\varphi_X(X_{it}) + \varepsilon^Y_{it}
\end{align*}

I can learn $\varphi_X$ from firms that do not overreport inputs $\varphi_{X|s}$ to avoid noise from $\varepsilon^X$
...
Markov assumption on productivity $\omega_{it}=\rho\omega_{it-1}+\eta_{it}$ with $\mathbb{E}[\eta_{it}]=0$

\begin{align}
  (\varphi_X(X_{it}) - &\rho\varphi_X(X_{it-1})) - (\varphi_X(X_{it-1}) - \rho\varphi_X(X_{it-2}))\\
  =& (\eta_{it}-\eta_{it-1})+(\varepsilon_{it}^X-\rho\varepsilon_{it-1}^X)-(\varepsilon_{it-1}^X-\rho\varepsilon_{it-2}^X) \\
\end{align}
...
\begin{align}
  \mathbb{E}[&\varphi_X(X_{1it})|S]-\mathbb{E}[\varphi_X(X_{0it})|S] \\
  &=\mathbb{E}[\varepsilon^X_{1it}-\varepsilon^X_{0it}|S]
\end{align}

Double differencing if productivity is linearly correlated with $S$.
...
1. $\varepsilon^X_{it}\perp\omega_{it-1}$ : Tax evasion independent of previous productivity
1. $\varepsilon^X_{it}\perp\varepsilon^Y_{it}$ : Tax evasion chosen before current period shock to output
...
2. $\mathbb{E}[\eta| X^*]$, $\eta\perp\varepsilon$
3. $f(y|x^*,x)=f(y|x^*)$
4. $G[f(x|x^*)]=x^*$

In my case, $m_0=\beta \ln X^* + \omega^{-1}(X^*)$ and $G[\cdot]=\inf\{\nu:\int_{\infty}^\nu f(x|x^*)dx \ge 0.9\}$
...
- Follow up with data: 
  ...
  - Mexico .small[follow up INAI request]
- Prospectus document and presentation
...
```{r, out.width="90%"}
library(qrcode)
qr <- qrcode::qr_code('https://raw.githack.com/hans-mtz/Slides/main/Slides/SNDR-08152022.html')
plot(qr)
