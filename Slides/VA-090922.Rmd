...
Where we left: 
- Identification strategy for different data scenarios
- Data
...
\begin{align*}
  ...
  \mathbb{E}[\ln Y_{it}|S_{it}>s]&=\mathbb{E}[\beta\ln(X^*_{it})|S_{it}>s] + \mathbb{E}[\omega_{it}|S_{it}>s] \text{(A-1)}\\
  &=\mathbb{E}[\beta\ln(X^*_{it})|S_{it}>s] + \mathbb{E}[g^{-1}(X^*_{it})|S_{it}>s] \text{(Monotonicity)}\\
...
From **GNR** 1st stage: $Var[\varepsilon^Y_{it}|S_{it}>s]=Var[\varepsilon^Y_{it}]$

Also, $Var[\ln Y_{it}|t]=Var[\beta \ln X^* + \omega|t]+Var[\varepsilon^Y_{it}|t]$
...
$Var[\ln Y_{it}|t]-Var[\varepsilon^Y_{it}|t]=(1-\beta)^{-2}Var[\omega_{it}|t]$

$Var[\ln Y_{it}|t]-Var[\varepsilon^Y_{it}|t]=Var[\ln X^*_{it}|t]$
...
\begin{equation*}
  V^E=\max_{\varepsilon^X_{it}} (1-\tau)\left(P_t y_{it} - \rho x_{it}^*\right)+\rho_t\tau \varepsilon^X_{it}-C(\varepsilon^X_{it}, s_{it})
\end{equation*}
